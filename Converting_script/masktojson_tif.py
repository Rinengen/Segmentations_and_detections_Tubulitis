...
def images_annotations_info(maskpath):
    ...
    for category in category_ids.keys():
        for mask_image in glob.glob(os.path.join(maskpath, category, f'*.{MASK_EXT}')):
            ...
            mask_image_open = tiff.imread(mask_image)
            mask_image_open = mask_image_open[3].astype(np.uint8)
            mask_image_open = np.where(mask_image_open>0,1,0)
            print(mask_image_open.shape)

            
            # Get image dimensions
            height, width = mask_image_open.shape
            ...
            thresh = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY,255,11)
...
if __name__ == "__main__":
    train_mask_path = "C:\\Users\\minik\\Desktop\\tif_tubules\\tif_mask"
    train_json_path = "C:\\Users\\minik\\Desktop\\tif_tubules\\tif_images\\1.json"
    process_masks(train_mask_path, train_json_path)

    test_mask_path = "C:\\Users\\minik\\Desktop\\datasets\\all_tubuli\\test\\masks\\"
    test_json_path = "C:\\Users\\minik\\Desktop\\datasets\\all_tubuli\\test\\images\\test.json"
    process_masks(test_mask_path, test_json_path)

    val_mask_path = "C:\\Users\\minik\\Desktop\\datasets\\all_tubuli\\val\\masks\\"
    val_json_path = "C:\\Users\\minik\\Desktop\\datasets\\all_tubuli\\val\\images\\val.json"
    process_masks(val_mask_path, val_json_path)


    aug_mask_path = "C:\\Users\\minik\\Desktop\\datasets\\all_tubuli\\train_aug\\masks\\"
    aug_json_path = "C:\\Users\\minik\\Desktop\\datasets\\all_tubuli\\train_aug\\images\\train_aug.json"
